...
class BulletEnv(gym.Env):
    ...
    def __init__(self, isGUI=False):
        ...
        self.time_step = 0.001 # for mpc, one timestep of force for policy = 5 timesteps of force for mpc
    ...
    def step(self, action):
        ...
        #print(self.current_joint_angles[1])
        ...
        reward = 0
        info = {}
        if isDone: info={"step":self.current_timestep}
    ...
    def getState(self, action):
        ...
        # applied_force = 3500 # somewhat max for our trained policy
        # applied_force = 0
        ...
        if (self.start <= self.current_timestep <= self.start + self.duration):

            # print(f"{self.current_timestep}, APPLY {self.force} FORCE")

            #Push random
            p.applyExternalForce(objectUniqueId=self.robot, linkIndex=-1, forceObj=[self.force[0], self.force[1], 0], posObj=[0, 0, 0], flags=p.LINK_FRAME)
