...
class SimpleRobot(object):
  ...
  def Step(self, action, start, duration, x, y):
    """Steps simulation."""
    ...
    self.step += 1
    #applied_force = 2000 # default
    #if(self.step % 250 in {0,1,2,3,4} and self.step > 5):
    if (start*5 <= self.step <= start*5 + duration*5):
    #if (start <= self.step <= start + duration):

        #print(f"{self.step}, APPLY {(x,y)} FORCE")
        ...
        #Push right
        #self.pybullet_client.applyExternalForce(objectUniqueId=1, linkIndex=-1, forceObj=[0, -applied_force, 0], posObj=[0, 0, 0], flags=self.pybullet_client.LINK_FRAME)
    done = False
    if self.step == 800 or not self.isHealthy():
    #if self.step == 400 or not self.isHealthy(): # 400 * 5 because 400 is max timestep i have for policy, *5 for frequency adjustment
    #if self.step == 1000 or not self.isHealthy():
      done = True
