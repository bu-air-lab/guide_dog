import re
import numpy as np
import time ## added this for gui##
import math
import random
random.seed(10)
...
class SimpleRobot(object):
  ...
  def _StepInternal(self, action, motor_control_mode):
    ...
    base_pos = self.pybullet_client.getBasePositionAndOrientation(1)[0] # planeid=0, robotid=1
    camInfo = self.pybullet_client.getDebugVisualizerCamera()
    curTargetPos = camInfo[11]
    distance = camInfo[10]
    yaw = camInfo[8]
    pitch = camInfo[9]
    self.pybullet_client.resetDebugVisualizerCamera(distance, yaw, pitch, base_pos)
    ## 
  ...
  def Step(self, action, force_vector, force_duration, force_start):

    #Determine which timesteps the force should be applied during
    force_start_timestep = int(force_start/(self.time_step * ACTION_REPEAT))
    force_end_timestep = int((force_start + force_duration)/(self.time_step * ACTION_REPEAT))


    for i in range(ACTION_REPEAT):

      #Apply force if timestep is within force window
      if(self.step >= force_start_timestep and self.step <= force_end_timestep):
        self.pybullet_client.applyExternalForce(objectUniqueId=self.quadruped, linkIndex=-1, forceObj=[force_vector[0], force_vector[1], 0], posObj=[0, 0, 0], flags=self.pybullet_client.LINK_FRAME)
    ...
    #if(start*4 <= self.step <= start*4 + duration*4):
    #  self.pybullet_client.applyExternalForce(objectUniqueId=1, linkIndex=-1, forceObj=[x, y, 0], posObj=[0, 0, 0], flags=self.pybullet_client.LINK_FRAME)

    #done = False
    #if self.step == 800 or not self.isHealthy():
    #  done = True
  ...
  def isHealthy(self):
    ...
    #Check balance
    isBalanced = True
    _, orientation = self.pybullet_client.getBasePositionAndOrientation(1)
    euler_orientation = self.pybullet_client.getEulerFromQuaternion(orientation)
    if(euler_orientation[0] > 0.4 or euler_orientation[1] > 0.2):
        isBalanced = False

    #Check if anything except feet are in contact with ground
    isContact = False
    contact_points = self.pybullet_client.getContactPoints(1, 0)
    links = []
