...
class EvalRobustnessEnv(gym.Env):
    ...
    def step(self, action, force_vector, force_duration, force_start):
        ...
        force_start_timestep = int(force_start/(self.time_step * self.action_repeat))
        force_end_timestep = int((force_start + force_duration)/(self.time_step * self.action_repeat))

        #Take action_repeat number of simulation steps to complete action
        for i in range(self.action_repeat):

            #Apply force if timestep is within force window
            if(self.current_timestep >= force_start_timestep and self.current_timestep <= force_end_timestep):
                p.applyExternalForce(objectUniqueId=self.robot, linkIndex=-1, forceObj=[force_vector[0], force_vector[1], 0], posObj=[0, 0, 0], flags=p.LINK_FRAME)
    ...
    # return robot pid for mpc controller:
    def robot_info(self):
        return self.robot

    def get_test_info(self):
        return self.force, self.start, self.duration
